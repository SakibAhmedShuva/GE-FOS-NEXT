...
export default function ChallanBuilderClient({ initialProject = null }: { initialProject?: ChallanProject | null }) {
  ...
  async function exportDoc(kind: "pdf" | "xlsx") { if (!projectId) { setError("Save challan first"); return; } setBusy(true); setError(null); try { const res = await fetch(`/api/exports/challan/${kind}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId }) }); const body = await res.json(); if (!res.ok) throw new Error(body.error || "Export failed"); const warnings = Array.isArray(body.warnings) && body.warnings.length ? ` Warning: ${body.warnings.join(" ")}` : ""; setMessage(`Generated ${kind.toUpperCase()} ${body.export.filename}.${warnings}`); window.open(body.downloadUrl, "_blank"); } catch (err) { setError(err instanceof Error ? err.message : "Export failed"); } finally { setBusy(false); } }
