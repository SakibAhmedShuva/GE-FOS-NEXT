...
export async function saveOfferProject({
  ...
  return prisma.$transaction(async (tx: any) => {
    ...
    return tx.project.findUnique({
      where: { id: project.id },
      include: { items: { orderBy: { sortOrder: "asc" } }, offerSetting: { include: { selectedCover: true } }, shares: { select: { sharedWithUserId: true, permission: true } }, owner: { select: { id: true, name: true, email: true, signatureStorageKey: true } } },
    });
  });
}
